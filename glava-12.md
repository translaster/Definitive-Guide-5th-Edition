...
| `AGENTLOGIN`     | Пишется при входе агента в систему. Поле `bridged channel` будет содержать что-то вроде `Agent/9994`, если войти в систему с помощью `change_agent`, а поле первого параметра будет содержать вошедший канал (например, `SIP/0000FFFF0001`). |
...
| `SYSCOMPAT`      | Записываается если агент пытается ответить на вызов, но не удается установить вызов из-за несовместимости в настройке медиапотока. |
