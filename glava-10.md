...
Если выражение принимает значение "истина", вызывающий объект отправляется в _`destination1`_. Если выражение оказывается ложным - вызывающий объект отправляется во второе назначение. Итак, что же такое истина и что такое ложь? Пустая строка и число `0` оцениваются как ложь. Все остальное оценивается как истина.
...
Короче говоря, `GotoIfTime()` отправляет вызов на указанный _`label`_, если текущая дата и время соответствуют критериям, указанным _`times, days\_of\_week, days\_of\_month`_ и _`months`_. Давайте рассмотрим каждый аргумент более подробно:
...
Вы также можете проверить значение данного ключа из командной строки Asterisk, запустив команду `database get` _`family key`_. Чтобы просмотреть все содержимое базы данных AstDB, используйте команду `database show`.
...
[6](https://learning.oreilly.com/library/view/asterisk-the-definitive/9781492031598/ch10.html%22%20/l%20%22idm46178406763720-marker) Мы понятия не имеем, как реализовать Пасху, но открыты для предложений.
