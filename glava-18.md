...
<table border="1" width="100%" cellpadding="5">
  <tr>
    <td>
      ...
      <p>Чтобы использовать асинхронный AGI, учетная запись AMI должна иметь разрешение agi как на чтение, так и на запись. Например, следующий пользователь определенный в <i>manager.conf</i> будет иметь возможность как а) выполнять действия диспетчера AGI, так и б) получать события AGI:</p>
      <p><pre><code>
...
В предыдущем разделе были рассмотрены возможные варианты использования учи. Этот раздел содержит более подробные сведения о том, как ваше пользовательское приложение AGI взаимодействует с Asterisk один раз АГИ() был вызван.

### Настройка сеанса AGI

Однажды `AGI()` или `EAGI()` была вызвана из диалплана Asterisk, некоторая информация передается в приложение AGI для настройки сеанса. В этом разделе рассматривается, какие шаги предпринимаются в начале сеанса AGI для различных вариантов.
...
| :--------- | :-------------- | :------- |
| `agi_request` | `hello-world.sh` | Первый аргумент, который был передан в приложение `AGI()` или `EAGI()`. Для process-based AGI это имя выполненного приложения aGI. Для FastAGI это будет URL-адрес, который был использован для подключения к серверу FastAGI. |
| `agi_channel` | `SIP/0004F2060EB4-00000009` | Имя канала, который выполнил команду приложения `AGI()` или `EAGI()`. |
